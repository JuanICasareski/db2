...
> Nota: si se exige no perder *ningún* evento ante caída de un nodo, puede subirse a **W=2** (escritura en mayoría) manteniendo R=1 → durabilidad más fuerte sin penalizar la lectura.
...
```
 ...
 └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ └────────────────┘
        |               |               |                |                   |
 Particionado:    Replicado para   Consultas por    Escritura masiva    Datos agregados
 por tenant +     lectura rápida   tenant + rol      partición por       para análisis
 process_id       (CP, R+W>N)      (índice por rol)  tenant/instancia/   (AP, eventual)
 (documental,     master-slave                       fecha (columnar,    series temporales
 strong)                                             AP, W bajo)
```
...
> *Aclaración: la arquitectura es conceptual. No exige implementar microservicios reales ni desplegar clusters.*
...
> Contrapunto: el **estado de instancia** es el más difícil en *consistencia* (requiere strong/CP), pero los **eventos** son los más difíciles en *volumen y escritura*, que es lo que la pregunta de escalabilidad pone en el centro.
...
**Documento preparado para:** entrega de 4–5 páginas + exposición oral de 4 minutos.
Cubre las Partes A–G y los criterios de evaluación de la Actividad 4 (volumen 15%, particionamiento 25%, hotspots 15%, replicación y N/R/W 25%, plan de escalabilidad 10%, diagrama y exposición 10%).
